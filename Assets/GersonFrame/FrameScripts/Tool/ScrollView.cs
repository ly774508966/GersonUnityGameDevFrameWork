...
namespace GersonFrame
{
    ...
    public class ScrollView : ScrollRect
    {
        ...
        public void ScrollTo(int fromindex, int targetindex,int maxAmMove)
        {
            if (fromindex < 0) fromindex = 0;
            if (targetindex < 0) targetindex = 0;

            int distance = targetindex - fromindex;
            if (distance<=0)
                ScrollTo(targetindex);
            else
            {
                if (Math.Abs(distance) > maxAmMove)
                    distance = distance > 0 ? maxAmMove : -maxAmMove;
                float localx = content.localPosition.x - defaultItemSize.x * distance;
                content.DOLocalMoveX(localx, 0.5f).onComplete = () => ScrollTo(targetindex);
