...
public class AES
{
    ...
    /// 加密字节头
    /// </summary>
    private const string AESHead = "GersonEncrypt";
    ...
    public static void AESFileEncrypt(string path, string EncrptyKey)
    {
        ...
        try
        {
            using (FileStream fs = new FileStream(path, FileMode.OpenOrCreate, FileAccess.ReadWrite))
            {
                if (fs != null)
                {
                    int count = AESHead.Length;
                    //读取字节头，判断是否已经加密过了
                    byte[] headBuff = new byte[count];
        ...
        {
            Debug.LogError("AESFileEncrypt " + path + " " + e);
        }
    }
    ...
    public static void AESFileDecrypt(string path, string EncrptyKey)
    {
        ...
        {
            Debug.LogError("AESFileDecrypt " + path + " " + e);
        }
    }
    ...
    public static byte[] AESFileByteDecrypt(string path, string EncrptyKey)
    {
        ...
        try
        {
            using (FileStream fs = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                if (fs != null)
                {
                    ...
                    {
                        Debug.LogWarning(string.Format("文件 {0}未加密 ", path));
                    }
                }
            }
        }
        ...
        {
            Debug.LogError("AESFileByteDecrypt error" + path + " " + e);
        }
        return DecBuffer;
    }
