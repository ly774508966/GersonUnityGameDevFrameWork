...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        static JsonMapper()
        {
            ...
            //==================解决float double 类型问题
            RegisterExporter<float>((obj, writer) => writer.Write(Convert.ToDouble(obj)));
        ...
        private static object ReadValue(Type inst_type, JsonReader reader, string filedname = null)
        {
            reader.Read();
                ...
                reader.Token == JsonToken.Boolean)
            {
                ...
                if (conv_op == null)
                {
                    ...
                    {
                        UnityEngine.Debug.LogError(string.Format("类型转换失败 字段名={0} 类型 {1} 值={2} 值类型={3}   请检查表对应的字段和值类型", filedname, inst_type, reader.Value, json_type));
                    }
                }

                UnityEngine.Debug.Log(conv_op.Invoke(null,
                                           new object[] { reader.Value }).GetType());
            ...
            else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                if (value_type is ILRuntime.Reflection.ILRuntimeType || value_type.FullName == "ILRuntime.Runtime.Intepreter.ILTypeInstance")
                    ...
                    if (t_data.Properties.ContainsKey(property))
                    {
                        ...
                        if (prop_data.IsField)
                        {
                            ((FieldInfo)prop_data.Info).SetValue(
                                instance, ReadValue(prop_data.Type, reader, property.ToString()));
                        }
                        else
                        {
                            PropertyInfo p_info =
        ...
        public static string ToJson(object obj)
        {
            lock (static_writer_lock)
            {
                ...
                string jsonstr = static_writer.ToString(); //System.Text.RegularExpressions.Regex.Unescape(static_writer.ToString());
                return jsonstr;
            }
        }
