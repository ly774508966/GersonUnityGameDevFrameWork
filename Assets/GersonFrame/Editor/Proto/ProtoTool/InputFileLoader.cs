    ...
    public static class InputFileLoader
    {
        ...
        private static string CompileDescriptor(string path, TextWriter stderr, params string[] args)
        {
            ...
            try
            {
                ...
                protocPath = UnityEngine.Application.dataPath + "/GersonFrame/Editor/Proto/ProtoTool/protoc.exe";
                protocPath = protocPath.Replace("/", "\\");

                string oursecondoutpath = Path.GetDirectoryName(protocPath); // secondary search path
             //   string inputprotopath = Path.Combine(Environment.CurrentDirectory, path);// input file
                string comdstr = string.Format(@"""--descriptor_set_out={0}"" ""--proto_path={1}"" ""--proto_path={2}"" --error_format=gcc ""{3}"" {4}",
                                tmp, // output file
                                Environment.CurrentDirectory, // primary search path
                              oursecondoutpath, // secondary search path
                                path, // input file
                                string.Join(" ", args) // extra args
                       );

                ProcessStartInfo psi = new ProcessStartInfo(protocPath, comdstr);
